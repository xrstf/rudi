...
func (i *interpreter) EvalTuple(ctx types.Context, tup ast.Tuple) (types.Context, any, error) {
	...
	resultCtx, result, err := i.CallFunction(ctx, identifier, tup.Expressions[1:])
	if err != nil {
		return ctx, nil, err
	}
...
func (*interpreter) CallFunction(ctx types.Context, fun ast.Identifier, args []ast.Expression) (types.Context, any, error) {
	funcName := fun.Name
